...
from config.query_logger import log_query  # ✅ added
...
if run_button and user_question:
    ...
    try:
        try:
            ...
            log_query(question=user_question, sql="", status="failed", message=f"SQL generation failed: {e}")
        ...
        try:
            ...
            if not rbac_status:
                log_query(question=user_question, sql=sql_query, status="failed", message=error_msg)
                st.error(f"Permission error: {error_msg}")
                raise RuntimeError(error_msg)

        except Exception as e:
            log_query(question=user_question, sql=sql_query, status="failed", message=f"RBAC validation failed: {e}")
            ...
            if i > 0:
                if ai_error:
                    try:
                        ...
                        st.warning("SQL correction failed")
                else:
        ...
        log_query(question=user_question, sql="", status="failed", message=str(e))
